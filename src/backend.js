...
export async function auth(kind,email,password){if(!navigator.onLine)throw Error('Connect to the internet to set up or recover your account.');if(!fb||session.connectionError)throw Error('The owner must configure the online account service first. Please complete Firebase setup.');if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email||''))throw Error('Enter your account email address.');await fb.authenticate(kind,email,password);}
...
export async function chat(guideId,message,reset=false){if(!cloud){const result=device.guide(session,guideId,message);await saveDevice(result.save);return {text:result.text};}signed();const result=await fb.call('askGuide',{guideId,message,reset});await refresh();return result;}
export async function fight(worldId,answer,choice,check,scenarioId){if(!cloud){const result=device.battle(session,worldId,answer,choice);await saveDevice(result.save);return result;}signed();const result=await fb.call('battle',{worldId,answer,choice,check,scenarioId,expectedRound:session.game.bosses[worldId]?.round||0});await refresh();return result;}
